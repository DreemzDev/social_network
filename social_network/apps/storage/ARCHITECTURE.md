...
Настройки читаются **при вызове**, а не константами уровня модуля. Ранняя
редакция вычисляла их один раз при импорте: `override_settings` в тестах
на них не действовал (тест квоты был вынужден подменять атрибут модуля
руками), а смена квоты требовала перезапуска процесса — вопреки
обещанию «меняется одной строкой» абзацем выше.
