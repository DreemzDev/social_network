...
class StorageService:
    ...
    def get_archive_response(items, *, filename: str):
        ...
        Права проверяет вызывающий модуль (ARCHITECTURE.md, раздел 8).

        Стриминг, потому что архив в памяти — это его полный размер на каждый
        параллельный запрос. `ZIP_STORED` без сжатия: на pdf/docx/картинках
        deflate даёт единицы процентов за процессорное время воркера.
        """
