...
Здесь только сам архив: разбор, проверки безопасности и цикл «запись →
`StorageService.upload()`». Записи и папки создаёт потребитель через
переданные `ensure_folder` и `create_record` — storage не знает его моделей
(ARCHITECTURE.md, раздел 1).

Архив приходит от пользователя, поэтому имена в кодировке MS-DOS, пути с
'..', степень сжатия и общий объём проверяются до записи первого байта.
"""
import os
import tempfile
import zipfile
...
def decode_entry_name(info: zipfile.ZipInfo) -> str:
    ...
    Без флага 0x800 `zipfile` декодирует по стандарту (cp437), а
    windows-архиваторы писали русские имена в cp866 — «Приказы 2024.pdf»
    приезжает как «╧ЁштъЁ√ 2024.pdf». Отсюда обратная перекодировка; на
    чистом ASCII она ничего не меняет.
    """
