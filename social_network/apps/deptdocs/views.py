...
class DeleteDepartmentFolderView(LoginRequiredMixin, View):
    ...
    Раньше удалить папку из интерфейса было нельзя вообще: оставался
    только /admin/, где каскад DepartmentFolder → DepartmentDocument
    сносил документы физически, минуя корзину.

    Здесь, в отличие от обменника и каталога, содержимое нельзя просто
    отвязать от папки: у DepartmentDocument права наследуются от папки, и
    folder=None означает «не виден никому» (см. докстринг модели). Такой
    документ пропал бы и из корзины, которая фильтруется по
    folder__allowed_users. Поэтому документы поддерева переезжают в
    РОДИТЕЛЬСКУЮ папку и там помечаются удалёнными: права родителя
    заведомо есть у того, кто удаляет — доступ к вложенной папке без
    доступа к родителю получить неоткуда, а выдаёт его тот же список
    allowed_users.

    У корневой папки родителя нет, девать документы некуда, и удаление
    отклоняется с объяснением: молча стереть их навсегда — ровно то
    поведение, от которого здесь и уходим.
    """
