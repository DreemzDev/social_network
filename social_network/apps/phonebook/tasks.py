...
В Celery, а не в вьюхе: LibreOffice запускается отдельным процессом и на
большом документе думает секунды-десятки секунд — держать на этом
HTTP-запрос значит показывать пользователю зависшую страницу.
"""
import os
import tempfile
