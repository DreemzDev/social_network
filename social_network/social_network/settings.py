...
# Путь к LibreOffice для конвертации справочников в PDF (storage/convert.py).
# None — искать самому в PATH и в обычных местах установки: на Астре пакет
# уже стоит, а на машине разработчика путь может быть свой. Если LibreOffice
# не найден, портал не показывает кнопку «Показать в браузере» — вместо неё
# остаётся скачивание файла.
STORAGE_SOFFICE_PATH = None
